...
function Index() {
  return <>
    <section className="relative min-h-[calc(92svh-4.5rem)] overflow-hidden bg-fir">
      ...
      <div className="relative mx-auto flex min-h-[calc(92svh-4.5rem)] max-w-7xl items-end px-5 pb-12 pt-20 sm:pb-16 sm:pt-28 lg:px-8 lg:pb-20">
        <div className="max-w-3xl animate-gentle-rise text-secondary-foreground">
          ...
          <p className="mt-6 max-w-xl text-base leading-7 opacity-80 sm:text-lg">Место, где мороз остаётся за дверью, а внутри пахнет свежей выпечкой, хвоей и только что смолотым кофе.</p>
           <div className="mt-8 grid max-w-sm gap-3 min-[370px]:grid-cols-2 sm:mt-9 sm:max-w-none sm:flex sm:flex-wrap"><Button asChild variant="warm" size="roomy"><Link to="/offers">Зимнее меню <ArrowRight /></Link></Button><Button asChild variant="winter" size="roomy"><Link to="/addresses"><MapPin /> Найти кофейню</Link></Button></div>
        </div>
      </div>
    </section>
    <section className="bg-background"><div className="mx-auto grid max-w-7xl gap-10 px-5 py-16 sm:py-20 lg:grid-cols-2 lg:gap-12 lg:px-8 lg:py-28">
    ...
    <section className="bg-wine text-accent-foreground"><div className="mx-auto grid max-w-7xl items-center gap-10 px-5 py-14 sm:py-16 lg:grid-cols-[1.05fr_.95fr] lg:gap-12 lg:px-8 lg:py-20">
    ...
    <section className="bg-background"><div className="mx-auto max-w-7xl px-5 py-16 sm:py-20 lg:px-8"><div className="grid grid-cols-[minmax(0,1fr)_auto] items-end gap-4"><div className="min-w-0"><p className="text-xs font-semibold uppercase text-primary">Свежие истории</p><h2 className="mt-4 font-display text-4xl">Новости кофейни</h2></div><Link to="/news" className="hidden shrink-0 items-center gap-2 text-sm font-semibold sm:flex">Все новости <ArrowRight size={16}/></Link></div><div className="mt-10 grid gap-px bg-border md:grid-cols-3">{["Новое зерно из Эфиопии","Вечера винила по пятницам","Зимняя выпечка уже на витрине"].map((title,i)=><article key={title} className="min-w-0 bg-background p-6 sm:p-7"><p className="text-xs text-muted-foreground">{["12 февраля","7 февраля","29 января"][i]}</p><h3 className="mt-5 font-display text-2xl leading-snug">{title}</h3><p className="mt-4 text-sm leading-6 text-muted-foreground">Загляните к нам за новыми вкусами и тихими зимними вечерами.</p></article>)}</div></div></section>
  </>;
}
